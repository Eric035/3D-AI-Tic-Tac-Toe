...
void greetAndInstruct() {
    ...
    while (true) {
        cin >> userInput;
        if (userInput == 'n') {
            // When user clicks 'n', program will be terminated
            cout << "Good bye :)" << endl;
            exit(0);
        }
        else if (userInput == 'y') {
...
// CheckIfLegal for Computer's move
bool compCheckIfLegal(int cellNum, char board[]) {
    // Check if user inputs an illegal cell number for the move (out of bound) or the cell is already occupied

    if (cellNum >= 0 && cellNum < 27) {  // This means the user desired cell number is in the range of possible cells
        if (board[cellNum] == ' ') {
            return true;
        }
        else {
...
bool checkIfLegal(int cellNbre, char board[]) {
    ...
    cout << "WARNING: Please input a cell number between 1 and 27." << endl;
    return false;
}

// Auxilary function for checkWinner (Horizontal cases)
bool checkHorzitonal(char board[], char whoWon[]) {
    ...
    for (int j = 0; j < 9; j += 3) {
        ...
        else if ((board[(j + 2)] == board[(j + 2) + 8]) && (board[(j + 2) + 8] == board[(j + 2) + 16]) && board[j + 2] != ' ') {
            whoWon[0] = board[j];
            return true;
        }
    }
    ...
    for (int k = 18; k < 27; k += 3) {
        ...
        else if ((board[k + 2] == board[(k + 2) - 10]) && (board[(k + 2) - 10] == board[(k + 2) - 20]) && (board[k + 2] != ' ')) {
            whoWon[0] = board[k];
            return true;
        }
    }
...
bool checkWinner(char board[]) {
    ...
    if (checkHorzitonal(board, whoWon)) {
...
void computerMove(char board[]) {
    ...
    for (int i = 0; i < 27; i++) {
        ...
        // cout << "Checking '0' optimal move: @ cell number " << i << endl;

        if (!compCheckIfLegal(i, board)) {      // If the cell is not available we skip this index and check the next cell
            continue;
        }
        ...
        // Check for horizontal win
        if (checkHorzitonal(board, whoWon) && whoWon[0] == '0') {
    ...
    for (int i = 0; i < 27; i++) {
        ...
        // cout << "Checking 'X' optimal move: @ cell number " << i << endl;
        ...
        // Check for horizontal win
        if (checkHorzitonal(board, whoWon) && whoWon[0] == 'X') {
    ...
    // cout << "No optimal move for either 'X' or '0'" << endl;
    ...
    for (int i = 0; i < 27; i++) {
        ...
        // cout << "Picking random cell: " << i << endl;

        if (compCheckIfLegal(i, board)) {
            board[i] = '0';
            break;
        }
    }
